  ...
  try {
    ...
    try {
      const cl = await pool.query(
        `SELECT cl.*, cc.total_credits, p.project_name, p.project_type,
         ...
         WHERE cl.status = 'active' AND cl.credits_for_sale > 0
    ...
    if (!rows.length) {
      ...
      const params = [];
      if (project_type) { params.push(project_type); q += ` AND sp.project_type=$${params.length}`; }
      if (min_price)    { params.push(parseFloat(min_price)); q += ` AND sp.price_per_credit>=$${params.length}`; }
      if (max_price)    { params.push(parseFloat(max_price)); q += ` AND sp.price_per_credit<=$${params.length}`; }
      if (vintage) {
...
router.get('/calculate', auth, async (req, res) => {
  const { baseline_emission, annual_reduction, leakage, buffer_percent } = req.query;
  try {
...
router.post('/',
  ...
  async (req, res) => {
    ...
    const {
      ...
      baseline_emission, annual_reduction,
      leakage = 0, buffer_percent = 10,
      price_per_credit, vintage_start, vintage_end,
      project_boundary, verification_doc_url,
      credits_for_sale,
    } = req.body;

    const net_credits = Math.max(0,
    ...
    try {
      ...
      const sp = await pool.query(
        ...
        [
          ...
          parseFloat(baseline_emission), parseFloat(annual_reduction),
          parseFloat(leakage), parseFloat(buffer_percent),
          net_credits, parseFloat(price_per_credit),
          parseInt(vintage_start), parseInt(vintage_end),
          project_boundary || null, verification_doc_url || null,
        ]
      );
      ...
      ['baseline_emission','annual_reduction','leakage','buffer_percent',
       'credits_available','price_per_credit'].forEach(f => {
        spRow[f] = Number(spRow[f] ?? 0);
      });

      // Also try writing to new carbon_credits + credit_listings tables
      try {
        const cc = await pool.query(
          `INSERT INTO carbon_credits
             (project_id, total_credits, available_credits, price_per_credit)
           VALUES ($1,$2,$3,$4) RETURNING id`,
          [spRow.id, net_credits, credits_for_sale || net_credits, parseFloat(price_per_credit)]
        );
        await pool.query(
          `INSERT INTO credit_listings
             (credit_id, seller_id, credits_for_sale, price_per_credit, status)
           VALUES ($1,$2,$3,$4,'active')`,
          [cc.rows[0].id, req.user.id, credits_for_sale || net_credits, parseFloat(price_per_credit)]
        );
      } catch {
        // carbon_credits / credit_listings not yet created — OK
      }
    ...
    try {
      ...
      await client.query(
        `INSERT INTO carbon_transactions
         ...
         VALUES ($1,$2,$3,$4,$5,$6)`,
        [req.user.id, p.user_id, project_id, parseFloat(credits), parseFloat(p.price_per_credit), total]
      );
