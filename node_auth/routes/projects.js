...
router.post('/',
  ...
  [
    ...
    body('project_type').notEmpty().withMessage('project_type required'),
  ],
  async (req, res) => {
    ...
    try {
      ...
      const result = await pool.query(
        `INSERT INTO projects
         ...
         RETURNING *`,
        hasGeom
          ? [req.user.id, project_name, project_type, description || null,
...
router.post('/:id/emission', auth, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.error('Python engine error:', pyErr.response?.data || pyErr.message);
    ...
    return res.status(500).json({ error: 'Failed to save emission', detail: err.message });
  }
});
...
router.post('/:id/absorption', auth, async (req, res) => {
  ...
  try {
    ...
    // Call Python engine for absorption calculation
    let calcData = null;
    try {
      const fastapiRes = await axios.post(
        `${FASTAPI}/api/v1/emission/calculate`,
      ...
      // Non-fatal: allow manual co2_absorbed if engine is down
      console.warn('Python engine unavailable for absorption, using manual value');
    }

    const absorbedValue = calcData
      ? (calcData.total_absorption_co2e || 0)
      : (parseFloat(co2_absorbed) || 0);

    const recordYear = year || new Date().getFullYear();

    // Insert into absorptions table (doc schema)
    await pool.query(
      `INSERT INTO absorptions (project_id, co2_absorbed, area_hectares, year)
       VALUES ($1,$2,$3,$4)
       ON CONFLICT DO NOTHING`,
      [project_id, absorbedValue, area_hectares || null, recordYear]
    ).catch((e) => console.warn('absorptions table not ready:', e.message));
    ...
    return res.status(500).json({ error: 'Failed to save absorption', detail: err.message });
  }
});
...
router.get('/:id/absorption', auth, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM absorptions WHERE project_id=$1 ORDER BY year DESC',
      [req.params.id]
    ).catch(() => ({ rows: [] }));
    return res.json(result.rows);
  } catch (err) {
    return res.status(500).json({ error: 'Failed to fetch absorption records' });
  }
});
