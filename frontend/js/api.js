...
const api = {
  ...
  listUsers: (role)  => api._fetch("GET", "/api/users" + (role ? `?role=${role}` : "")),

  // ── 3. PROJECT SERVICE ─────────────────────────────────────────────────────
  createProject:         (body) => api._fetch("POST", "/api/projects", body),
  getProject:            (id)   => api._fetch("GET",  `/api/projects/${id}`),
  submitProjectEmission: (id, body) => api._fetch("POST", `/api/projects/${id}/emission`, body),
  getProjectEmission:    (id)       => api._fetch("GET",  `/api/projects/${id}/emission`),
  submitProjectAbsorption: (id, body) => api._fetch("POST", `/api/projects/${id}/absorption`, body),
  getProjectAbsorption:    (id)       => api._fetch("GET",  `/api/projects/${id}/absorption`),

  // ── 4. ACCOUNTING SERVICE (Python engine via Node proxy) ──────────────────
  // Primary buyer emission calculation — existing UI uses this, keep as-is
  calculateEmission: (body) => api._fetch("POST", "/api/carbon/emission/calculate", body),
  // Absorption calculation
  calculateAbsorption: (params) =>
    api._fetch("GET", "/api/carbon/absorption/calculate?" + new URLSearchParams(params)),

  // ── 5. TRANSACTION + MARKETPLACE SERVICE ──────────────────────────────────
  // Listings
  getListings: (params) =>
    api._fetch("GET", "/api/credits/listing" + (params ? "?" + new URLSearchParams(params) : "")),
  ...
  gisDistricts:    (params) => api._fetch("GET", "/api/gis/districts"      + (params ? "?" + new URLSearchParams(params) : "")),
  gisEmissionMap:  (params) => api._fetch("GET", "/api/gis/emission-map"   + (params ? "?" + new URLSearchParams(params) : "")),
  gisProjectsMap:  ()       => api._fetch("GET", "/api/gis/projects-map"),
  gisAbsorptionMap:(params) => api._fetch("GET", "/api/gis/absorption-map" + (params ? "?" + new URLSearchParams(params) : "")),
  ...
  dashboardYearly: (params) => {
    ...
    return api._fetch("GET", "/api/dashboard/yearly" +
      (Object.keys(filtered).length ? "?" + new URLSearchParams(filtered) : ""));
  },
  ...
  dashboardRegional: (params) => {
    ...
    return api._fetch("GET", "/api/dashboard/regional" +
      (Object.keys(filtered).length ? "?" + new URLSearchParams(filtered) : ""));
  },
  ...
  dashboardSummary: (params) => {
    ...
    return api._fetch("GET", "/api/carbon/dashboard/summary" +
      (Object.keys(filtered).length ? "?" + new URLSearchParams(filtered) : ""));
  },
  dashboardRegion: (params) =>
    api._fetch("GET", "/api/carbon/dashboard/region?" + new URLSearchParams(params || {})),
  districts: (params) =>
    api._fetch("GET", "/api/carbon/districts?" + new URLSearchParams(params || {})),

  // ── Legacy Seller aliases (seller_form.html uses /api/seller/project) ─────
  registerSellerProject: (body) => api._fetch("POST", "/api/seller/project", body),
